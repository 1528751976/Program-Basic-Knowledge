...
- [顺序表与链表的区别：](#顺序表与链表的区别)        
- [链表的分类：](#链表的分类)        
- [顺序表的操作：(其本质就是一个数组)](#顺序表的操作其本质就是一个数组)      
- [顺序表的方面的练习题：](#顺序表的方面的练习题)        
- [单链表的操作：](#单链表的操作)        
- [双链表的操作：](#双链表的操作)        
- [链表的习题：](#链表的习题)       
- [其他重要知识点](#其他重要知识点)

[第二章  栈与队列](#第二章--栈与队列)<!-- /TOC -->
...
2. 举例说明两个数据结构有相同的逻辑结构和存储方式，但运算定义不同，且两个结构是不同的数据结构。
    答：例如：二叉树和二叉排序树
        二叉树：n个结点的集合，当n=0时，为空二叉树；当n>0时，它由一个根结点和两颗互不相交的称为左右子树的二叉树形成，二叉树的定义是一个递归定义，通常用爱表示层次。
        二叉排序树：建立在二叉树的基础上，然后满足如下性质：

   * 若它的左子树非空，则左子树上所有记录的值均小于根记录的值。
   * 若它的右子树非空，则右子树上所有记录的值均大于根记录的值。
   * 左右子树本身又各是一颗二叉排序树。

    两者在逻辑结构和存储方式上完全相同，因为二叉排序树完全采用二叉树的逻辑表示和存储方式。两者的运算有建立树、插入节点、删除结点、查找结点等。对于二叉树和二叉排序树，这些运算的定义不同。以查找结点为例，二叉树的时间复杂度为O（n）；而二叉排序树的时间复杂度为O（$log_2{n}$）
